...
function initRevealAnimations() {
  ...
  // Reveal-up elements (story-blocks, etc.)
  ...
  document.querySelectorAll(".portfolio-list").forEach(list => {
    ...
    gsap.to(visibleRows, {
      y: 0, opacity: 1, duration: 0.65, stagger: 0.045, ease: "power2.out",
      scrollTrigger: { trigger: list, start: "top 90%", once: true }
    });
  });
}

// ────────────────────────────── 3. Portfolio row hover ──────────────────────────────
...
function initGraphicHover() {
  ...
  document.querySelectorAll(".graphic-card").forEach(card => {
    ...
    card.addEventListener("mouseenter", () => {
      gsap.fromTo(shine, { right: "-60%" }, { right: "130%", duration: 1, ease: "power2.inOut" });
    });
  });
}
...
function initExpandableLists() {
  document.querySelectorAll("[data-toggle-list]").forEach(button => {
    ...
    if (!extraItems.length) { button.style.display = "none"; return; }
    let visibleCount = 0;
    const batchSize = 5;
      ...
      requestAnimationFrame(() => {
        ...
        if (typeof gsap !== "undefined") {
          batchItems.forEach((item, i) => {
            ...
            gsap.to(item, {
              ...
              duration: 0.65, delay: i * 0.06, ease: "power3.out",
              onComplete: () => {
        ...
        if (label) {
          ...
          if (left === 0 && typeof gsap !== "undefined") {
            gsap.to(button, { opacity: 0, duration: 0.3, onComplete: () => button.style.display = "none" });
          } else if (left === 0) {
...
function initMagneticButtons() {
  ...
  document.querySelectorAll(".cta-btn").forEach(btn => {
    btn.addEventListener("mousemove", e => {
      ...
      gsap.to(btn, { x: x * 0.08, y: y * 0.12, duration: 0.3, ease: "power2.out" });
    });
...
function initGlowPulse() {
  ...
  if (!glow) return;
  gsap.to(glow, { x: 28, y: 18, opacity: 0.8, duration: 2.2, yoyo: true, repeat: -1, ease: "sine.inOut" });
}
